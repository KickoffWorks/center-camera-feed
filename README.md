...
The extension detects video elements on the page and creates a floating copy at your chosen position. For self-view, it finds your muted video stream. For active speaker, it uses platform-specific selectors to identify who's currently speaking, with fallback detection based on video size.

The original videos remain in place (so the platform continues to work normally), while you get a convenient reference point near your camera.
...
- Only clones existing video elements that are already on the page
